...
public class TupleParser {
	 ...
	 */
	public void parse(ListNode list, String stmt) throws ScriptException {
		...
		try {
			for(;;) {
				...
				try{
					...
					throw new ScriptException(e, linecnt, stmt);
				}
				if(line==null)
					break;
				...
				if(tp.length!=2)
					throw new ScriptException("'"+line+"' is not a tuple.", linecnt, stmt);
